...
function VFXController:SetupLightShafts()
    local shaftConfig = Config.Lighting.LightShafts
    if not shaftConfig then return end
    ...
    for i = 1, shaftConfig.Count or 10 do
        local shaft = Instance.new("Part")
        shaft.Name = "LightShaft"
        shaft.Size = shaftConfig.Size or Vector3.new(10, 500, 10)
        -- Random position within a reasonable range around the origin for the demo
        shaft.Position = Vector3.new(math.random(-400, 400), 200, math.random(-400, 400))
        shaft.Orientation = Vector3.new(0, 0, 15) -- Slight tilt for cinematic effect
        shaft.CastShadow = false
        shaft.CanCollide = false
        shaft.Anchored = true
        shaft.Material = Enum.Material.SmoothPlastic
        shaft.Transparency = shaftConfig.Transparency or 0.95
        shaft.Color = shaftConfig.Color or VFXUtil.Colors.ElectricBlue
        shaft.Parent = folder
        
        -- Subtle pulse to make them feel alive
        VFXUtil.Pulse(shaft, shaft.Color, Color3.new(1, 1, 1), 6 + math.random() * 4)
    end
end
