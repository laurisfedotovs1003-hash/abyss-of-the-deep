--[[
	CreatureService — Handles creature spawning, AI behavior, and catch mechanics
...
local activeEncounteredCreatures = {} -- { [playerUserId] = { creature = ..., isShiny = bool } }
...
function CreatureService:ProcessEncounters()
	for userId, encounter in pairs(activeEncounteredCreatures) do
		-- De-spawn creatures that have been active too long
...
function CreatureService.Client:RequestCatch(player)
	...
	if success then
		...
		if encounter.isShiny then
			sellPrice = sellPrice * 3 -- Shiny bonus
		end
		...
		self.Client:Get("CreatureCaught"):Fire(player, {
			...
			size = creatureDef.Size,
			sellPrice = sellPrice,
			xpReward = rarityConfig.XPMultiplier * Config.Economy.XPPerCreatureCaptured,
		})
		...
		return {
			...
			xpGained = rarityConfig.XPMultiplier * Config.Economy.XPPerCreatureCaptured,
		}
	else
		-- Creature escapes but stays for another attempt
		return { result = "Failed", reason = "It got away!" }
	end
end
